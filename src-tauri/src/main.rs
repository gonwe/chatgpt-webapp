// Tauri 桌面封装 — 由 tauri-aur-scaffold 生成
// 注意: 此文件为模板生成，修改后建议同步更新生成器模板 templates/main.rs
...
(function() {
    ...
    // NOTE: this script runs at document start (UserScriptInjectionTime::Start),
    // so document.head may not exist yet — guard against it.
    ...
    function setup() {
        ...
        function hideBtn() {
            ...
            for (var i = 0; i < btns.length; i++) {
                if (btns[i].textContent.trim() === HIDE_TEXT) {
